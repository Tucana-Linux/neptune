...
def calculate_removed_dependencies(packages_to_remove):
   future_wanted_packages = set(open(f"{functions.settings.lib_dir}/wanted_packages", "r").read().splitlines())
...
def remove():
   ...
   wanted_packages = set(open(f"{functions.settings.lib_dir}/wanted_packages", "r").read().splitlines())
   ...
   subprocess.run(f"sed -i '/{package}/d' {functions.settings.lib_dir}/wanted_packages", shell=True)
