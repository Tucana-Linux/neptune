...
class Frontend:
    ...
    def install(self):
        ...
        packages_to_install = self.system.utils.get_depends(
            ...
            installed_packages=self.system.installed_packages,
        )
    ...
    def update(self):
        updates = self.system.utils.check_for_updates(
            installed_packages=self.system.installed_packages,
            versions=self.system.versions,
        )
        recalculated_depends = self.system.utils.recalculate_system_depends(
            self.system.wanted_packages, self.system.installed_packages
        )
    ...
    def remove(self):
        ...
        for package in packages_user_wants_removed:
            ...
            if not (package in self.system.wanted_packages):
                print(
                    f"{package} was installed as a dependency of another package, it can not be removed sanely"
                )
                sys.exit(1)
        ...
        absolute_packages_to_remove: list[str] = (
            self.system.utils.recalculate_system_depends(
                ...
                self.system.installed_packages,
            )[1]
        )
